...
namespace irubataru.tinyPlayer {

    class Song {
        public title: string;
        public files: string[];
        public html_elem: JQuery;
        public preload: boolean;
        public howl: Howl

        constructor(title: string, files: string[], html_elem: any, preload: boolean, howl: Howl) {
            this.title = title;
            this.files = files;
     ...
     */
    class Player {
        public _playlist: Song[];
        public _index: number;
        public _mouse_down: boolean;
        public _seeking: boolean;
        public _animation_timestamp: number;
         ...
         */
        seek(index: number, percentage: number) {
            var self = this;
            ...
            html_elem.find(".song-timer").html(formatTime(seek_to) +
                " / " +
         ...
         */
        updateDuration(index: number, percentage: number) {
            var self = this;
            var html_elem = self._playlist[index].html_elem;
        ...
        step(timestamp: any) {
            ...
            html_elem.find(".song-timer").html(formatTime(seek) +
                " / " +
                formatTime(sound.duration()));
     ...
     */
    export function createPlayerFromTags() {
        var playlist: Song[] = [];
        ...
        $("audio.iru-tiny-player").each(function () {
            ...
            var song_elem = createSongPlayer(title);

            playlist.push(new Song(title, files, song_elem, preload, null));
     ...
     */
    function createSongPlayer(title: string) {
        return $("<div>")
     ...
     */
    function setupEvents(player: Player) {
        var song_index = 0;

        player._playlist.forEach(function (song: Song) {
            var song_elem = song.html_elem;
            var this_idx = song_index;

            song.howl = createHowlForPlayer(player, song);
            bindPlayerControls(player, song_elem, this_idx);
     ...
     */
    function createHowlForPlayer(player: Player, song: Song) {
        ...
        function onLoad() {
            var self: Howl = this;
            song_elem.find(".song-timer").html(formatTime(
                self.duration()));
        }
     ...
     */
    function bindPlayerControls(player: Player, elem: JQuery, idx: number) {
        // Bind the play button
        elem.find(".fa-play").on("click",function () {
            player.toggleTo(idx);
        });

        // Bind the pause button
        elem.find(".fa-pause").on("click", function () {
            player.pause(idx);
        });

        // Bind the stop button
        elem.find(".fa-stop").on("click", function () {
            player.stop(idx);
        });

        // Show the volume control
        elem.find(".fa-volume-up").on("click", function () {
        ...
        elem.find(".song-volume-bar#fgg").on("click", function (event) {
            var x = event.pageX;
            x = x - elem.find(".song-volume-bar#bg").offset().left -
                7.5;
            var width = parseFloat(String(elem.find(".song-volume-bar#bg").innerWidth()));
            var per = x / width;
        ...
        elem.find(".song-volume-dot").on("mousedown", function () {
            player._mouse_down = true;
        });

        // Click end the volume dot
        elem.find(".song-volume-control").on("mouseup", function () {
            player._mouse_down = false;
        });

        // Update sound level as you move the sound dot
        elem.find(".song-volume-control").on("mousemove", function (event) {
        ...
        // currently is.
        function seekPercentage(event: JQuery.Event, elem: JQuery, object_class: string) {
            var x = event.pageX;
            x = x - elem.find(object_class).offset().left;
            var width = parseFloat(String(elem.find(".song-seek").innerWidth()));
            return Math.min(1, Math.max(0, x / width));
        }

        // Click start on seek
        elem.find(".song-seek").on("mousedown", function (event) {
            player._seeking = true;
            player.updateDuration(idx, seekPercentage(event, elem,
                ".song-seek"));
        });

        // Update the visual seek position as you move the cursor over the seek
        // area.
        elem.find(".song-seek").on("mousemove", function (event) {
            if (player._seeking) {
                player.updateDuration(idx, seekPercentage(event, elem,
                    ".song-seek"));
            }
        ...
        elem.find(".song-seek").on("mouseup", function (event) {
            player._seeking = false;
            player.seek(idx, seekPercentage(event, elem, ".song-seek"));
        });

        // TODO Find out how I can avoid having duplicate code
        // Copy of the same functions for .song-progress
        elem.find(".song-progress").on("mousedown", function (event) {
            player._seeking = true;
            player.updateDuration(idx, seekPercentage(event, elem,
                ".song-progress"));
        });

        elem.find(".song-progress").on("mousemove", function (event) {
            if (player._seeking) {
                player.updateDuration(idx, seekPercentage(event, elem,
                    ".song-progress"));
            }
        ...
        elem.find(".song-progress").on("mouseup", function (event) {
            player._seeking = false;
            player.seek(player._index, seekPercentage(event, elem, ".song-progress"));
        });
...
(function() {
  ...
  document.addEventListener("DOMContentLoaded", (event) => {
      irubataru.tinyPlayer.createPlayerFromTags();
  })
